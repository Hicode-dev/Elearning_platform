...
const Testimonials = () => {
  const { data, error } = useFetch("http://localhost:1234/testimonials");
  return (
    <div className="mt-10">
      ...
      <p className="text-gray-600 lg:text-xl md:text-xl text-medium   mt-4 text-start w-9/12 mx-auto">
        Discover why more than 20,000 students have entrusted their education to
        us, and join our esteemed learning community today
      </p>
          ...
          data.map((el, i) => (
            <div key={i} className="shadow p-2 rounded-xl">
              <div className="flex items-center ">
                <img className="w-11 h-11 rounded-full" src={el.image} alt="" />
                <div className="grid ml-3">
                  <div className="flex items-center ">
                    <h6 className="w-full font-bold text-blue-900 text-xs ">{el.name}</h6>
                    {/* svg for each users */}
                    <div class="flex items-center ">
                      ...
                      <span class="text-gray-400 text-xs font-semibold   ml-3">
                        (4.5)
                      </span>
                    </div>
                  </div>
